...
function Necessity:new(config)
    local resource = {
        ...
        label = config.label,
        decrement = config.decrement or 1,
...
function Necessity:increment(player)
    ...
    local next_value = current_value + self.decrement
...
local function OnLogin(event, player)
    local summaries = {}
    for _, key in ipairs(BareNecessitiesResourceOrder) do
        table.insert(summaries, BareNecessitiesResources[key]:summary(player))
    end
